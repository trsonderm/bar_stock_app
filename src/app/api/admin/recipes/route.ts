...
export async function GET(req: NextRequest) {
    ...
    if (source === 'local' || source === 'both') {
        ...
        const rows = await db.query(
            `SELECT id, name, description, ingredients, instructions, category, tags, is_active, created_at, updated_at FROM org_drink_recipes ${where} ORDER BY name ASC`,
            params
        );
    ...
    if (source === 'global' || source === 'both') {
        ...
        if (category) { params.push(category); where += ` AND category = $${params.length}`; }
        const rows = await db.query(
            `SELECT id, name, description, ingredients, instructions, category, tags, is_active, created_at, updated_at FROM drink_recipes ${where} ORDER BY name ASC`,
            params
        );
