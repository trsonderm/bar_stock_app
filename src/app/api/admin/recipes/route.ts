...
export async function GET(req: NextRequest) {
    ...
    if (source === 'local' || source === 'both') {
        ...
        const rows = await db.query(
            `SELECT id, name, description, ingredients, instructions, category, glass, amount, tags, is_active, created_at, updated_at FROM org_recipes ${where} ORDER BY name ASC`,
            params
        );
    ...
    if (source === 'global' || source === 'both') {
        ...
        if (category) { params.push(category); where += ` AND category = $${params.length}`; }
        const rows = await db.query(
            `SELECT id, name, description, ingredients, instructions, category, glass, amount, tags, is_active, created_at, updated_at FROM recipes ${where} ORDER BY name ASC`,
            params
        );
...
export async function POST(req: NextRequest) {
    ...
    markChanged(session.organizationId, 0, 'recipes');
