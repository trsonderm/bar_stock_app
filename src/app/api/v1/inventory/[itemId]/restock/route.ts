/**
 * POST /api/v1/inventory/:itemId/restock
 * Add stock for a specific item at a location.
 *
 * Body:
 *   quantity     number   required  Amount to add (must be positive)
...
export async function POST(req: NextRequest, { params }: { params: { itemId: string } }) {
    ...
    try {
        ...
        const updated = await client.query(
            `UPDATE inventory SET quantity = quantity + $1, organization_id = $4
        ...
        await client.query(
            `INSERT INTO activity_logs (organization_id, user_id, action, details) VALUES ($1, NULL, 'ADD_STOCK', $2)`,
            [session.organizationId, JSON.stringify({
                itemId, itemName: item.name,
                quantityAdded: quantity, quantityAfter, locationId: targetLocationId,
        ...
        return apiOk({
            ...
            quantity_before: quantityAfter - Number(quantity),
