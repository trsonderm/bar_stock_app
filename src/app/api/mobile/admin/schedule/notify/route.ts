/**
 * POST /api/mobile/admin/schedule/notify
 *
 * Send schedule notification emails to one or more employees.
 * Body:
 *   userIds         — integer[]
 *   scheduleEntries — { date, shiftName, startTime, endTime, color? }[]
 *   message         — optional string prepended to the email
 *
 * Requires admin role.
 */
...
export async function POST(req: NextRequest) {
    ...
    const { userIds, scheduleEntries, message } = await req.json();
    if (!Array.isArray(userIds) || userIds.length === 0) {
        return NextResponse.json({ error: 'userIds[] required' }, { status: 400 });
    }

    const orgRow = await db.one('SELECT name FROM organizations WHERE id = $1', [session.organizationId]);
