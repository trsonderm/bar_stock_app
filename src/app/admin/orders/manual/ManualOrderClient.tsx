...
export default function ManualOrderClient({ user }: { user: any }) {
    ...
    const submitOrder = async () => {
        ...
        // We can integrate with actual order submission API later.
        console.log('Submitting Order:', orderItems);
        alert('Order submitted successfully! (This is a placeholder for the actual API)');
        setCart({});
        ...
        <div className={styles.container}>
            ...
            {showPreview && (
                <div style={{ position: 'fixed', top: 0, left: 0, right: 0, bottom: 0, background: 'rgba(0,0,0,0.8)', display: 'flex', justifyContent: 'center', alignItems: 'center', zIndex: 100 }}>
                    <div style={{ background: '#111827', padding: '2rem', borderRadius: '1rem', width: '90%', maxWidth: '600px', border: '1px solid #374151', maxHeight: '90vh', overflowY: 'auto' }}>
                        <h2 style={{ color: 'white', marginTop: 0 }}>Order Preview</h2>
                        <table style={{ width: '100%', color: 'white', textAlign: 'left', borderCollapse: 'collapse', marginBottom: '1.5rem' }}>
                            <thead>
                                <tr style={{ borderBottom: '1px solid #374151' }}>
                                    <th style={{ padding: '0.5rem' }}>Product</th>
                                    <th style={{ padding: '0.5rem', textAlign: 'center' }}>Supplier Order Qty</th>
                                    <th style={{ padding: '0.5rem', textAlign: 'center' }}>Stock Addition</th>
                                </tr>
                            </thead>
                            <tbody>
                                {Object.values(cart).map((c, i) => (
                                    <tr key={i} style={{ borderBottom: '1px solid #1f2937' }}>
                                        <td style={{ padding: '0.5rem' }}>{c.itemName} ({c.packAmount})</td>
                                        <td style={{ padding: '0.5rem', textAlign: 'center', fontWeight: 'bold', color: '#34d399', fontSize: '1.1rem' }}>{c.orderQty}</td>
                                        <td style={{ padding: '0.5rem', textAlign: 'center', color: '#9ca3af' }}>+{c.orderQty * c.packAmount} units</td>
                                    </tr>
                                ))}
                            </tbody>
                        </table>
                        <div style={{ display: 'flex', justifyContent: 'flex-end', gap: '1rem' }}>
                            <button onClick={closePreview} style={{ padding: '0.5rem 1rem', background: 'transparent', color: '#9ca3af', border: '1px solid #374151', borderRadius: '0.5rem', cursor: 'pointer' }}>Cancel</button>
                            <button onClick={submitOrder} style={{ padding: '0.5rem 1rem', background: '#10b981', color: 'white', border: 'none', borderRadius: '0.5rem', cursor: 'pointer', fontWeight: 'bold' }}>Confirm Order</button>
                        </div>
                    </div>
                </div>
            )}
