...
interface BlockMeta {
    ...
    help: string;
    unique?: boolean; // only one of this type allowed
}
...
const DEFAULT_TEMPLATE: CalculatorTemplate = {
    blocks: [
...
function StaffPreview({ blocks }: { blocks: TemplateBlock[] }) {
    ...
    return (
        <div style={{ background: '#0a0f1a', borderRadius: '0.75rem', padding: '1.25rem', border: '1px solid #1f2937' }}>
            <div style={{ color: '#9ca3af', fontSize: '0.75rem', textTransform: 'uppercase', letterSpacing: '0.05em', marginBottom: '1rem' }}>Staff View Preview</div>
            {blocks.map((b, i) => {
...
export default function ShiftCalculatorBuilder() {
    ...
    useEffect(() => {
        fetch('/api/admin/shift-calculator')
            .then(r => r.json())
            .then(d => {
                if (d.template?.blocks?.length) {
                    setBlocks(d.template.blocks);
                    setHasTemplate(true);
                } else {
                    setBlocks(DEFAULT_TEMPLATE.blocks);
                }
            })
    ...
    const handleSave = async () => {
        ...
        try {
            await fetch('/api/admin/shift-calculator', {
                ...
                body: JSON.stringify({ template: { blocks, updatedAt: new Date().toISOString() } }),
            });
            setHasTemplate(true);
            setSaved(true);
    ...
    return (
        <div style={cs.page}>
            ...
            {/* Build tab */}
            {activeTab === 'build' && (
                <div style={{ display: 'grid', gridTemplateColumns: '260px 1fr', gap: '1.5rem', maxWidth: '1100px', margin: '0 auto', padding: '1.5rem 1rem', alignItems: 'start' }}>
                    ...
                    <div>
                        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '1rem' }}>
                            <div>
                                <div style={{ color: '#f9fafb', fontWeight: 600, fontSize: '1rem' }}>Calculator Layout</div>
                                <div style={{ color: '#6b7280', fontSize: '0.8rem' }}>{blocks.length} block{blocks.length !== 1 ? 's' : ''} — click a label to edit it</div>
                            </div>
                            {blocks.length > 0 && (
                                <button
                                    onClick={() => { if (confirm('Clear all blocks?')) setBlocks([]); }}
                            ...
                            <>
                                ...
                                {blocks.map((block, idx) => (
                                    <div key={block.id}>
                                        ...
                                        {idx < blocks.length - 1 && block.type !== 'divider' && blocks[idx + 1].type !== 'divider' && (
                                ...
                                <div style={{ marginTop: '1.5rem', background: '#111827', border: '1px solid #1f2937', borderRadius: '0.75rem', padding: '1rem' }}>
                                    ...
                                    <div style={{ display: 'flex', flexWrap: 'wrap', gap: '0.4rem', alignItems: 'center' }}>
                                        {blocks.filter(b => b.type !== 'divider' && b.type !== 'result' && b.operation !== 'display_only').map((b, i) => (
                                        ...
                                        {blocks.some(b => b.type === 'result') && (
                                            <>
                                                <span style={{ color: '#6b7280', fontSize: '1.1rem' }}>=</span>
                                                {blocks.filter(b => b.type === 'result').map(b => (
                                        ...
                                        {blocks.every(b => b.operation === 'display_only' || b.type === 'divider' || b.type === 'result') && (
