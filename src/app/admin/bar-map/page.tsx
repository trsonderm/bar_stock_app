...
export default async function BarMapPage() {
    ...
    const [mapRow, productRows] = await Promise.all([
        ...
        db.query('SELECT id, name, type FROM products WHERE organization_id=$1 AND active=TRUE ORDER BY type, name', [session.organizationId]),
    ]);
    ...
    return (
        <div style={{ padding: '0 24px 24px' }}>
            <BarMapClient initialMap={initialMap} products={products} />
        </div>
    );
}
