...
export default function CustomReportClient() {
    ...
    const sendReportNow = async () => {
        if (rows.length === 0) return alert('No data to send. Please generate a report first.');
        
        const res = await fetch('/api/super-admin/reports/send', {
            method: 'POST',
        ...
        if (res.ok) alert('Report sent via Email successfully!');
                        ...
                        <div className="flex items-center gap-4">
                            ...
                            {rows.length > 0 && (
                                <button 
                                    ...
                                    className="bg-blue-600 hover:bg-blue-500 text-white px-4 py-1.5 rounded text-sm font-bold shadow-lg"
                                >
                                    Send Report Now
                                </button>
                            )}
